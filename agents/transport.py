...
def transport_agent(state: TripState) -> TripState:
    ...
    if not state.get("transport_options"):
        _add_default_transport_option(state, destination, constraints)
    ...
    print(f"transport_agent(): Coordinated {len(sub_agents_to_call)} sub-agents, found {len(state.get('transport_options', []))} transport options")
...
def _add_default_transport_option(
    state: TripState,
    destination: str,
    constraints: dict
):
    """
    Add a default transport option if sub-agents didn't produce any results
    ...
    state["transport_options"] = [default_option]